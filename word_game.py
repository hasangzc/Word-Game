...
def get_word_score(word, n):
    ...
    """
    word = word.lower()
    input_freq_dict = (get_frequency_dict(word))
    take_values = []
    for i in range(len(word)):
        a = SCRABBLE_LETTER_VALUES.get(word[i])
        take_values.append(a)
    comp_one = 0
    for x in range(0, len(take_values)):
        if take_values[x] != None:
            comp_one += take_values[x]

    # comp_one = sum(take_values)
   ...
   """
    ...
    for char in word:
        for i in range(len(x)):
            if x[i] == char and x[i - 1] != char:
                new_hand.remove(x[i])
    return get_frequency_dict(new_hand)
...
def is_valid_word(word, hand, word_list):
    ...
    if not (WILDCARD in word):
        ...
        t = False
        for v in VOWELS:
            if (word.replace(WILDCARD, v) in word_list):
                t = True
                break
        if not t:
            return False
    ...
    # never be deleted from the hand x = random.choice(VOWELS)
    #         hand[x] = hand.get(x, 0) + 1
    for lett in word:
...
def substitute_hand(hand, letter):
    ...
    if not letter in new_hand:
        ...
        options = (set(VOWELS) | set(CONSONANTS)) - set(new_hand.keys())
        ct = new_hand[letter]
        del(new_hand[letter])
        optionsString = "".join(options) # Set datatype doesn't support indexing!
        replacement = random.choice(optionsString)
        new_hand[replacement] = ct
        return new_hand
